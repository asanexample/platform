...
- **Images must be cosign-signed** (keyless; Enforce on preprod). App CI is a **thin caller** of the shared, app-team-unwritable `asanexample/trusted-ci/build-sign.yml` reusable workflow (build → push → sign → SBOM) + `slsa-provenance.yml` (provenance) — the supply-chain backbone is NOT copied per app (ADR-050; `app-bravo` is the generic starter). Kyverno's `verify-images-product-<team>-<product>` admits images signed by the shared `build-sign.yml` identity **gated to the product** by the cert's `githubWorkflowRepository` extension (= the app repo `app-<team>-<product>`); a per-product app-signed identity (the app repo's `deploy.yml`/`preview.yml`) is also accepted as a fallback for bespoke-build apps. Another team's image is rejected. Full explainer: `docs/architecture/cosign-image-signing.md`, `docs/runbooks/app-supply-chain-onboarding.md`.
