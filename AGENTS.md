...
- **Observability instrumentation golden path + OTLP↔Prometheus convergence (ADR-100)** — SDK-first is the paved road (the `internal/telemetry`-shape: OTel traces + metrics + Pyroscope profiles + trace-stamped structured logs); Beyla (eBPF) is a **zero-touch fallback for un-instrumented workloads only** — never both on one service (Beyla's context-propagation fights the SDK's `traceparent`). Mimir's OTLP ingest is configured (`otel_metric_suffixes_enabled`, `otel_keep_identifying_resource_attributes`, `promote_otel_resource_attributes` — a **CSV string**, not a list) to translate OTLP metrics to the Prometheus convention (`..._seconds`, `k8s_namespace_name`) on write, so a dashboard/SLO/canary query is identical regardless of delivery path. Gotcha: OTLP series break out by **`job`** (`<team>-<product>/<svc>`), not `service_name` — Mimir won't surface the latter as a label even when promoted.
...
- **PR preview environments (ADR-032)** — re-implemented on the v3 delivery model and **Shipped** (2026-07-01, proven end-to-end against a real PR): `argocd-apps`' `pr-preview.tf` deploys one `pullRequest`-generator ApplicationSet per preview-enabled Product (`products[*].preview`), landing each open PR's signed image into the existing `dev` namespace, isolated via kustomize `namePrefix`/`commonLabels`, at `<product>-<team>-dev-pr-<N>.<preview_domain>`, auto-cleaning up on PR close. Distinct from `preview_domain`'s per-*stage* hostname rewrite (the standard per-Product ApplicationSet), which is not per-PR.
