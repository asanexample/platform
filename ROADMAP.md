...
- [#1124](https://github.com/asanexample/platform/issues/1124) **Epic: close alerting blindspots (dead-man's switch, coverage, detection, routing).** The dead-man's switch is live (Watchdog → external heartbeat, #1128/#1131). **Done:** P0 CNPG backup/health alerting ([#1119](https://github.com/asanexample/platform/issues/1119) ✅ — scrape + backup-health/WAL-archiving/instance-down alerts, all three DBs); P1 blackbox external-reachability + public-cert-expiry alerts ([#1122](https://github.com/asanexample/platform/issues/1122) — the `blackbox.rules` group on the already-wired probes: `BlackboxProbeDown`/`Slow` + public `Ssl…Expiring` critical/warning); P0 severity re-triage + a **watched home for warnings** ([#1120](https://github.com/asanexample/platform/issues/1120) — the page-worthy warnings were already `critical`; warnings now open a LOW-urgency (non-paging) PagerDuty incident via the service's `severity_based` urgency rule, so they land in a reviewed queue instead of an unwatched Slack channel). **Remaining:** P1 semantic control-plane/stateful alerts ([#1121](https://github.com/asanexample/platform/issues/1121)); P2 breadth + hygiene ([#1123](https://github.com/asanexample/platform/issues/1123)).
