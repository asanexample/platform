...
| 2026-07-07 | 1.15    | J. Deeden | P13 scoped to done: hard isolation + grants **PROVEN on metrics + logs** (the two highest-value signals). Dashboard sharing needs no build — OSS Grafana has no per-team dashboard RBAC (Enterprise-only), so the grant itself is the mechanism. **Traces + profiles DEFERRED** as mechanical follow-ups (traces = costly OTel routing for low marginal value). Pivoting to the alerting-blindspots epic (#1124). |
| 2026-07-07 | 1.14    | J. Deeden | P13 **LOGS (Loki) hard isolation + grants LIVE**: preprod Alloy re-tenants env-namespace logs per team (system → cluster tenant), a `loki-tenant-proxy` (same grant-aware image, signal-agnostic) enforces identity-scoped reads, and the spoke edge passes the per-team tenant through (write-integrity mTLS deferred). Verified: hub Loki `alpha`/`bravo` tenants each see only their own namespaces. Remaining: traces, profiles, dashboard-sharing. |
| 2026-07-07 | 1.13    | J. Deeden | P13 metrics **cross-team read grants LIVE**: the tenant-proxy unions tenants granted via `AccessGrant` (ADR-068 — observability is its first consumer) into a federated read, regulated (pci/hipaa) owners excluded. Grant-aware proxy image deployed on the hub. Remaining P13: dashboard-sharing UX + logs/traces/profiles. |
...
| 2026-07-12 | 1.16    | J. Deeden | **P10 extended to tenant app-SDK OTLP metrics** ([ADR-100](docs/adrs/100-observability-instrumentation-and-otlp-convention.md)): a preprod `/otlp/v1/metrics` Gateway edge + Mimir OTLP-ingest translation to the Prometheus convention, so dashboards/SLOs/canary gates see one metric shape regardless of delivery path. **Compute & Elasticity (#643/ADR-078) moved Now → Shipped**: both phases now proven on a real tenant Product (alpha-shop) alongside its metric-gated canary (ADR-056) — first time either ran on a non-platform service — which surfaced and fixed a real gap (`argocd-apps` AppProject whitelist was missing `HorizontalPodAutoscaler`, silently failing every tenant HPA sync). |
| 2026-07-12 | 1.17    | J. Deeden | Added Developer Portal (Later): reconcile GitHub repo existence off the Product CR (Crossplane GitHub provider), surfaced while onboarding the new bravo-dispatch demo product by hand. |
| 2026-07-14 | 1.18    | J. Deeden | Onboarded **Pulse** (`platform-pulse`) — a synthetic-traffic generator Product that drives continuous demo traffic against alpha-shop and bravo-dispatch plus an on-demand SLO-alert trigger, mirroring flagship's platform-owned-Product shape. Surfaced and fixed two real platform bugs: an ADR-101 ServiceGrant Composition bug where two grants from one subject to different targets collided on the rendered ingress CNP's name (platform#1604), and a stale `observability-authoring` skill claim about per-team tenancy still being live (platform#1647; ADR-104 parked it back on 2026-07-13). Also added a fleet-wide Alerting Overview Grafana dashboard (sortable/filterable alert table + severity-colored firing-history timeline). |
...
- [#102](https://github.com/asanexample/platform/issues/102) Observability stack (epic) — metrics, logs, traces, profiles, cost. **Data plane COMPLETE — LGTM+P, multi-cluster, federated, correlated.** Live: P1 metrics+dashboards, P2 Mimir durable store, P3 logs+traces ([#582](https://github.com/asanexample/platform/issues/582)), P4 alerting + Slack/PagerDuty ([#583](https://github.com/asanexample/platform/issues/583)), P5 cloud-resource metrics ([#584](https://github.com/asanexample/platform/issues/584)), **P6 APM correlation** (service graph + exemplars + Traces Drilldown), **P7 instrumentation** (Beyla eBPF, [#586](https://github.com/asanexample/platform/issues/586) / [ADR-077](docs/adrs/077-application-instrumentation-strategy.md)), **P8 continuous profiling** (Pyroscope, both clusters, traces↔profiles — [#587](https://github.com/asanexample/platform/issues/587)/#637), **P9 SLOs + synthetics** (Sloth + blackbox + k6 — [#588](https://github.com/asanexample/platform/issues/588)), P10 metrics spoke + the cross-cluster Gateway edge generalized to logs/traces/profiles — **extended 2026-07-11/12 to tenant app-SDK OTLP metrics** (a preprod `/otlp/v1/metrics` Gateway edge + Mimir's OTLP ingest translated to the Prometheus convention on write, so a dashboard/SLO/canary query is one shape regardless of delivery path — [ADR-100](docs/adrs/100-observability-instrumentation-and-otlp-convention.md); the original P10 spoke only carried remote-write metrics, with no path for app-SDK metrics), P11 in-cluster cost ([#589](https://github.com/asanexample/platform/issues/589), shipped/closed) + true cloud cost CUR→Athena ([#668](https://github.com/asanexample/platform/issues/668), shipped/closed), P12 policy-reporter, and **Grafana SSO via Keycloak** ([#592](https://github.com/asanexample/platform/issues/592)/#638). **Remaining = the access plane: P13 per-team isolation ([#590](https://github.com/asanexample/platform/issues/590)) — METRICS hard isolation now ENFORCED (2026-07-07): all Grafana prometheus datasources route through the `observability-tenant-proxy` (`oauthPassThru` → identity-scoped `X-Scope-OrgID`), the un-proxied `mimir`/`mimir-preprod`/`mimir-all` bypass paths are gone, and the proxy is fail-closed (no/invalid token → 401). Cross-team read GRANTS are now LIVE for metrics too (2026-07-07): the proxy unions any tenants granted via an `AccessGrant` (ADR-068 — observability is its first consumer) into a federated read (`X-Scope-OrgID: alpha|bravo`), regulated (pci/hipaa) owner teams excluded. **LOGS (Loki) hard isolation + grants are now LIVE too (2026-07-07)**: preprod Alloy re-tenants env-namespace logs to their team (system logs → cluster tenant), a `loki-tenant-proxy` enforces identity-scoped reads, and the spoke edge passes the per-team tenant through (write-integrity via ingest mTLS deferred, #590 Phase-4 D-2). Verified live: the hub Loki `alpha`/`bravo` tenants each see only their own namespaces, `preprod` only system ones. **P13 hard isolation + grants are thus PROVEN on the two highest-value signals (metrics + logs).** Dashboard sharing needs no separate build — OSS Grafana shows all dashboards to everyone (per-team dashboard RBAC is Enterprise-only, ruled out), so the grant IS the mechanism: a granted team opens the owner's dashboard and its panels populate (un-granted → empty). Traces (Tempo) + profiles (Pyroscope) isolation are **deferred as mechanical follow-ups** — the pattern is proven; traces especially is high-cost (OTel routing-connector + per-team exporters) for low marginal value. Do them only if the "all four signals" completeness box is specifically needed.** + P14 self-service ([#591](https://github.com/asanexample/platform/issues/591) — needs re-scoping off the retired `teams.hcl` onto the v3 registries) + small polish (#151/#595/#93/#161, multi-account CloudWatch).
...
- [#1124](https://github.com/asanexample/platform/issues/1124) **Epic: close alerting blindspots (dead-man's switch, coverage, detection, routing).** The dead-man's switch is live (Watchdog → external heartbeat, #1128/#1131). **Done:** P0 CNPG backup/health alerting ([#1119](https://github.com/asanexample/platform/issues/1119) ✅ — scrape + backup-health/WAL-archiving/instance-down alerts, all three DBs); P1 blackbox external-reachability + public-cert-expiry alerts ([#1122](https://github.com/asanexample/platform/issues/1122) — the `blackbox.rules` group on the already-wired probes: `BlackboxProbeDown`/`Slow` + public `Ssl…Expiring` critical/warning). **Remaining:** P0 severity re-triage + a watched home for warnings ([#1120](https://github.com/asanexample/platform/issues/1120) — the *reclassify* half is effectively done: the page-worthy warnings the audit named are already `critical` and the rest are legitimately warning-tier. The *watched-home* half is **DEFERRED**: the intended fix (route warnings to a LOW-urgency PagerDuty queue) is blocked — **the PagerDuty trial expired**, so there is no PD account to page to. Revisit when PD returns, or pivot to a reviewed-Slack-with-ownership model); P1 semantic control-plane alerts ([#1121](https://github.com/asanexample/platform/issues/1121) — **partly shipped**: Karpenter (cloud-provider errors / stuck-Pending pods), ArgoCD controller-wide reconciliation-stalled (the 6-day-outage mode), and a generic controller-runtime reconcile-error alert (catches the activation-operator) — all grounded in live metrics. **Keycloak alerts NOW SHIPPED** (enabled `KC_METRICS_ENABLED` + a ServiceMonitor on the keycloak module — the metrics are scraped as `job=keycloak-http`; added `KeycloakDown` (critical — SSO down platform-wide) + `KeycloakHighErrorRate`; login-event metrics need KC's `user-event-metrics` feature, a follow-up). **Crossplane alerts SHIPPED** (code): enabled `metrics.enabled` on the crossplane chart + a PodMonitor in the observability module (mirrors the CNPG one — `kubernetes_manifest` can't live in the crossplane module's plan), and added `CrossplaneDown` (critical) + auto reconcile-error coverage via the generic `ControllerReconcileErrors`. Note: the crossplane unit is the highest-blast-radius apply (orphan-sweep null_resources), so rather than a standalone in-place apply, `metrics.enabled` is left to activate on the **next teardown/rebuild** — the crossplane unit is the highest-blast-radius apply (orphan-sweep IAM/ECR null_resources) and this defers the migration to the rebuild per the usual pattern. The observability side (PodMonitor + `CrossplaneDown`) is already live and self-arms when crossplane comes up with the metrics port; it's inert-but-waiting until then); P2 breadth + hygiene ([#1123](https://github.com/asanexample/platform/issues/1123) — **shipped**: closed the observation gaps from a coverage audit — a `mimir.rules` group (the one store without alerts), `tailscale.rules` (the private-access path — kube-state signals, critical on the subnet router), and `platform-services.rules` (cortex-tenant / argo-rollouts / Beyla / external-dns). Every workload also has the kube-prometheus-stack default net (crashloop/not-ready) + the 4 UIs are blackbox-probed). **Epic essentially closed** — only #1120 (watched-home-for-warnings) remains, deferred on the PagerDuty trial expiring. **⚠️ Paging plane status:** with the PagerDuty trial expired, per-alert **paging is currently down** (the PD-based on-call plane, ADR-084 Phase 2, is dormant) — alerts still flow to **Slack + SNS**, and the **external Healthchecks dead-man's switch** (#1128/#1131) still pages if the whole pipeline dies, but nothing pages an on-call human per-alert until PD is restored.

#### Compute & Elasticity

Moved to Shipped (below, Platform Tooling & Ops) — [#643](https://github.com/asanexample/platform/issues/643), both phases proven live on a platform service *and* a real tenant Product. **Remaining:** KEDA (event-driven — deferred until first needed per the ADR).
...
- [#629](https://github.com/asanexample/platform/issues/629) **Epic: single pane of glass — multi-cluster, all signals.** **Collection + read-federation DONE** — the Gateway-native spoke edge is replicated across every store and one Grafana spans platform + preprod, broken out by `cluster`, with correlation intact:
  ...
  - **Remaining = the access half: P13 per-team isolation ([#590](https://github.com/asanexample/platform/issues/590)) — METRICS hard isolation ENFORCED (2026-07-07); grants + other signals in flight.** Decision: re-tenant by team (hard boundary; Enterprise ruled out). Shipped 2026-07-02/03: write-path re-tenant (`observability-cortex-tenant`), read-side identity→tenant proxy (`observability-tenant-proxy`) + per-team Grafana datasource, a second tenant for isolation testing, and per-team overview dashboards (#1126–#1157). **2026-07-07: the proxy is now the ENFORCED front door for metrics — every prometheus datasource routes through it (identity-scoped, fail-closed), no un-proxied bypass remains. Cross-team read GRANTS are LIVE for metrics (AccessGrant projection, ADR-068; regulated owners excluded). **LOGS (Loki) hard isolation + grants LIVE 2026-07-07** (preprod Alloy per-team re-tenant + `loki-tenant-proxy`; verified per-team on the hub Loki). Dashboard sharing = delivered by the grant (OSS Grafana has no per-team dashboard RBAC — Enterprise-only, ruled out; the grant gates the data). Traces (Tempo) + profiles (Pyroscope) DEFERRED as mechanical follow-ups (pattern proven on metrics+logs).** Full plan in the #590 design comment.
...
- **Reconcile GitHub repo existence off the Product CR** — Net-new. Today the New Product scaffolder's `publish:github` step creates the repo and the `platform-skeleton` PR registers it in one atomic click; the `gitops/products/**` CR only *records* `spec.repo`, nothing reconciles the repo back into existence if a Product is ever registered ahead of/without going through the scaffolder (hit manually while hand-authoring the bravo-dispatch registry, 2026-07-12). A Crossplane GitHub provider (or a small dedicated controller) could own repo existence + settings (visibility, branch protection) the same declarative way ECR/IAM/namespace are owned today. Scaffolding the starter *content* (language-specific skeleton) should stay Backstage's job — that's a one-time templated-generation action, not a continuously-reconciled desired state; only existence/settings would move to the reconciler.
...
- **Package registry: AWS CodeArtifact + ECR pull-through cache** — Shipped + live 2026-07-09 (epic #1249, [ADR-098](docs/adrs/098-package-registry-codeartifact.md)). The language-package sibling to ECR — private npm/PyPI/Maven/NuGet in per-Product repos with upstream-proxy caching of public sources — plus a public-image mirror via ECR pull-through cache (`registry.k8s.io` + `quay.io` live; `docker.io`/`ghcr.io` credential-gated). Auth reuses Pod-Identity (runtime baseline read) + GitHub-OIDC (CI publish); no self-hosted registry. Go modules unserved (D5).
...
- [#643](https://github.com/asanexample/platform/issues/643) **Cluster elasticity — Karpenter + workload autoscaling ([ADR-078](docs/adrs/078-cluster-elasticity-karpenter.md)).** Shipped, both phases. **Phase 1** — Karpenter node autoscaling on both clusters (conservative `on-demand`/`WhenEmpty` on the stateful platform hub; spot retired in favour of `on-demand`/`WhenEmptyOrUnderutilized` on preprod), with BYOCNI startup-taint ordering + `platctl` park-awareness. **Phase 2** — HPA on the paved road so the loop closes: HPA adds pods → Karpenter adds nodes → consolidation reclaims. Default HPA emitted by the golden path (ADR-078 D6): the New Product skeleton renders an `autoscaling/v2` HPA targeting the Rollout (CPU 70%, min 1 / max 10; prod patches min→2 for the ADR-085 replica floor); metrics-server is live (CPU source); opt-out, not opt-in. Proven live twice: a synthetic CPU-load test on the New Product skeleton drove an HPA 1→12 replicas → Karpenter provisioned a node (3→4) and consolidated it away on load-drop (2026-07-08); then proven **on a real tenant Product** (alpha-shop's storefront, 2026-07-11) alongside its metric-gated canary (ADR-056) — same rollout, same overlay, first time both ran together on a non-platform service. That proof surfaced a real gap: the per-Product ArgoCD AppProject `namespaceResourceWhitelist` was missing `HorizontalPodAutoscaler`, so ArgoCD silently rejected every tenant product's HPA (and, being an atomic sync, failed the whole overlay with it) until fixed (`argocd-apps` #1342). **Remaining:** KEDA (event-driven — deferred until first needed per the ADR).
