...
- [#629](https://github.com/asanexample/platform/issues/629) **Epic: single pane of glass — multi-cluster, all signals.** **Collection + read-federation DONE** — the Gateway-native spoke edge is replicated across every store and one Grafana spans platform + preprod, broken out by `cluster`, with correlation intact:
  ...
  - **Remaining = the access half: P13 per-team isolation ([#590](https://github.com/asanexample/platform/issues/590)) — BUILT FOR METRICS, INERT.** Decision: re-tenant by team (hard boundary; Enterprise ruled out). Shipped 2026-07-02/03: write-path re-tenant (`observability-cortex-tenant`), read-side identity→tenant proxy (`observability-tenant-proxy`) + per-team Grafana datasource, a second tenant for isolation testing, and per-team overview dashboards (#1126–#1157). **Still open: per-team read isolation for logs (Loki) / traces (Tempo) / profiles (Pyroscope)** — currently metrics-only — and the proxy is not yet the enforced front door. Full plan in the #590 design comment.
