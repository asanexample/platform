...
| `Team` | common.hcl (default) / resource-level | Owning environment team; `platform` for cross-cutting resources, `<team>` on team-owned ones (ECR repos, per-team IAM roles). Basis for per-team ABAC (ADR-039, #62). | `platform`, `alpha` |
