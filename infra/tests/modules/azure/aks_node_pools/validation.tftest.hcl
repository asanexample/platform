...
run "prefix_validation_test" {
  ...
  variables {
    prefix         = "test"  # Valid prefix between 1-10 chars
    environment    = "dev"
    region_abbv    = "eus"
...
run "customer_validation_test" {
  ...
  variables {
    ...
    environment    = "dev"
    region_abbv    = "eus"
    customer       = "customer1"  # Valid customer name
    aks_cluster_id = "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/test-rg/providers/Microsoft.ContainerService/managedClusters/test-dev-aks-eus"
    app_node_pool_enable_auto_scaling = false
    tags = {}
  }
  ...
  assert {
    condition     = var.customer == "customer1"
    error_message = "Customer name should be valid with proper format"
  }
}
