...
terraform {
  source = include.base.locals.module_source.observability_events
}
...
# Ships events to Loki — pull the push endpoint from the loki unit.
dependency "loki" {
  config_path = "../loki"
...
inputs = {
  # Events are part of log collection — gated by the same toggle as the pod-log DaemonSet + Loki store.
