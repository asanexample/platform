...
inputs = {
  ...
  # Trace -> logs link target (#1269): the FEDERATED `loki-all` datasource, not single-tenant `loki` (platform).
  # App logs are split into per-team Loki tenants by the spoke passthrough, so only the federated datasource
  # (needs Loki multi_tenant_queries_enabled, set in the loki module) finds a preprod trace's `alpha` logs.
