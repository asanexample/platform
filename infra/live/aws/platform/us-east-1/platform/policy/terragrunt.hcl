...
inputs = {
  ...
  # triage-copilot XAgent (gitops/agents/triage-copilot.yaml) is live. Rolling out Audit-FIRST: the policies
  # report any signature/attestation gap without blocking admission, so we confirm the agent's signed+attested
  # image passes in-cluster (Kyverno PolicyReports) before flipping verify_failure_action/attest_failure_action
  # to "Enforce" to match preprod. Attestation verification requires image verification (reuses the ECR-read IRSA).
