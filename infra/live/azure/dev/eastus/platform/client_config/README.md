# Azure Client Config - East US (Dev)

## Overview

This directory contains the Terragrunt configuration for retrieving the current Azure client configuration (subscription, tenant, etc.) in the East US region for the development environment. This module is used as a dependency by other modules that need information about the current Azure client context.
...
```bash
cd infra/live/azure/dev/eastus/client_config
terragrunt plan
terragrunt apply
```
