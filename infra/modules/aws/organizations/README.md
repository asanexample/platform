...
| <a name="input_tag_scp_exempt_roles"></a> [tag\_scp\_exempt\_roles](#input\_tag\_scp\_exempt\_roles) | IAM role names exempt from ONLY the two tag-governance SCPs (require-tagging + DenyTeamTagTampering), NOT the blanket exempt\_roles. For provisioners/controllers (e.g. nh-deployer, Karpenter) that must set governance tags on resources they create at creation time, yet remain subject to every OTHER guardrail — the ADR-017 two-axis / #072 finding: a tag-only exemption instead of blanket admin-exempt. | `list(string)` | `[]` | no |
