...
that grants a pod its role, and it is an AWS-API resource tenants cannot create, so it is the
platform-controlled, namespace+ServiceAccount-scoped isolation boundary (see ADR-041).

The module is team-agnostic: the per-team `associations` map is assembled at the terragrunt unit from
`teams.hcl`. The agent add-on (`eks-pod-identity-agent`) must be installed on the cluster (see the
