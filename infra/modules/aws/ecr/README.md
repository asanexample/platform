...
- **Pull-through cache** (`pull_through_cache_rules`) mirrors public registries into this account on first pull (ADR-098 D2). `registry.k8s.io` and `quay.io` support anonymous pull-through; Docker Hub and `ghcr.io` each **require** a Secrets Manager credential whose name starts with `ecr-pullthroughcache/` (AWS rejects the rule otherwise). The pulling principal needs `ecr:BatchImportUpstreamImage` + `ecr:CreateRepository`.
