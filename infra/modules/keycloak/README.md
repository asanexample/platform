...
| <a name="input_finalizer_clear_script"></a> [finalizer\_clear\_script](#input\_finalizer\_clear\_script) | Path to scripts/k8s-finalizer-clear.sh. When set (and the in-cluster DB is used), a destroy-time provisioner force-deletes the CNPG Cluster + its pods/PVCs so the namespace can finalize. Empty disables it. | `string` | `""` | no |
