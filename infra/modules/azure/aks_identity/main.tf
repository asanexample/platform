...
locals {
  ...
  aks_identity_name                      = var.aks_identity_name != null ? var.aks_identity_name : "${var.workload}-${var.environment}-aks-identity"
  cert_manager_identity_name             = var.cert_manager_identity_name != null ? var.cert_manager_identity_name : "${var.workload}-${var.environment}-cert-manager"
  cert_manager_federated_credential_name = var.cert_manager_federated_credential_name != null ? var.cert_manager_federated_credential_name : "${var.workload}-${var.environment}-cert-manager-fedcred"
  karpenter_identity_name                = var.karpenter_identity_name != null ? var.karpenter_identity_name : "${var.workload}-${var.environment}-karpenter"
  karpenter_federated_credential_name    = var.karpenter_federated_credential_name != null ? var.karpenter_federated_credential_name : "${var.workload}-${var.environment}-karpenter-fedcred"
}
...
resource "azurerm_role_assignment" "aks_subnet_network_contributor" {
  count                = var.create && var.subnet_id != null ? 1 : 0
  scope                = var.subnet_id
  role_definition_name = "Network Contributor"
  principal_id         = azurerm_user_assigned_identity.aks_identity[0].principal_id
}
...
resource "azurerm_user_assigned_identity" "cert_manager_identity" {
  count               = var.create && var.create_workload_identities && var.workload_identity_enabled && var.oidc_issuer_enabled ? 1 : 0
