...
output "aks_identity_id" {
  ...
  value       = var.create_aks_identity ? azurerm_user_assigned_identity.aks_identity[0].id : null
}

output "aks_identity_principal_id" {
  description = "Principal ID of the user-assigned managed identity for the AKS cluster."
  value       = var.create_aks_identity ? azurerm_user_assigned_identity.aks_identity[0].principal_id : null
}

output "aks_identity_client_id" {
  description = "Client ID of the user-assigned managed identity for the AKS cluster."
  value       = var.create_aks_identity ? azurerm_user_assigned_identity.aks_identity[0].client_id : null
}
...
output "cert_manager_identity" {
  ...
  value = contains(keys(azurerm_user_assigned_identity.workload_identities), "cert-manager") ? {
...
output "karpenter_identity" {
  ...
  value = contains(keys(azurerm_user_assigned_identity.workload_identities), "karpenter") ? {
