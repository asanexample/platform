...
| <a name="input_database"></a> [database](#input\_database) | Backstage database. mode = in-cluster (CloudNativePG) \| rds. For in-cluster, instances/storage\_size size the CNPG Cluster. | <pre>object({<br/>    mode         = optional(string, "in-cluster")<br/>    instances    = optional(number, 1)<br/>    storage_size = optional(string, "5Gi")<br/>  })</pre> | `{}` | no |
