...
| <a name="input_codeartifact_domain"></a> [codeartifact\_domain](#input\_codeartifact\_domain) | CodeArtifact domain (in the platform account, same account as the ECR registry) that holds the per-Product package repos (ADR-098). When non-empty, the Composition grants every service's Pod-Identity role baseline READ (cross-account) on this Product's consumer repo (refplat/<team>-<product>) so workloads can pull private + upstream-cached packages. Empty = grant nothing (renders identically to pre-ADR-098). | `string` | `""` | no |
