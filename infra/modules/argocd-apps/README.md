...
> **Note:** per-PR ephemeral preview environments (ADR-032) — a separate, product-scoped `pullRequest`-generator ApplicationSet (`pr-preview.tf`), gated per-product on `products[*].preview` — live, proven end-to-end against a real PR (`asanexample/alpha-shop#14`). See [Notes](#notes).
...
- AppProject whitelists are restrictive by design: ConfigMaps, Secrets, Services, **ServiceAccounts**, Deployments, StatefulSets, **Argo `Rollout`/`AnalysisTemplate`/`AnalysisRun`** (ADR-056 — every workload is a Rollout; Deployment/StatefulSet kept for the migration window), **`autoscaling/HorizontalPodAutoscaler`** (ADR-078 — the golden path scaffolds an HPA targeting the Rollout; omitting this from the whitelist silently rejects EVERY tenant's HPA sync, and since a sync is atomic, fails the whole overlay with it — bit alpha-shop's canary+HPA rollout, #1342), Jobs, CronJobs, HTTPRoutes, NetworkPolicies, and ExternalSecrets.
