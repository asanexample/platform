...
# ---------------------------------------------------------------------------
# WorkforceRole catalog registry-sync (ADR-088 / identity strategy §2.2, #887).
# Projects gitops/roles/*.yaml — the reach×power role catalog — onto the cluster
# as cluster-scoped WorkforceRole CRs. The WorkforceRole CRD ships with the
# crossplane environment-api chart; the Terraform IC/Keycloak generators keep
# reading the same git files, so this is a parallel READ-ONLY projection (git is
# the source of truth — prune/selfHeal reconciles any manual edit away), exactly
# like the Team sync above. First consumer: the activation operator, which reads
# spec.identityCenter.sessionDuration (the borrow cap) + .permissionSet at runtime.
# ---------------------------------------------------------------------------

resource "kubernetes_manifest" "roles_project" {
  for_each = var.enable_roles ? { enabled = true } : {}
  ...
  manifest = {
    ...
    spec = {
      description = "Git-native WorkforceRole catalog (platform-managed, ADR-088 / #887)"
      sourceRepos = [var.roles_repo_url]
      destinations = [{
        server    = var.cluster_server
        namespace = "crossplane-system"
      }]
      # Only the cluster-scoped WorkforceRole — nothing else.
      clusterResourceWhitelist = [
        { group = "platform.refplat.org", kind = "WorkforceRole" },
      ]
      namespaceResourceWhitelist = []
    }
  }
...
resource "kubernetes_manifest" "roles_app" {
  for_each = var.enable_roles ? { enabled = true } : {}
  ...
  manifest = {
    ...
    metadata = {
      ...
      namespace = var.argocd_namespace
      labels = {
        "platform.refplat.org/component" = "roles"
      }
      # Same early wave as Teams — the catalog is an admission/runtime input.
      annotations = {
        "argocd.argoproj.io/sync-wave" = "-1"
      }
    }
    spec = {
      ...
      source = {
        repoURL        = var.roles_repo_url
        targetRevision = var.roles_repo_branch
        path           = var.roles_repo_path
      }
      destination = {
        server    = var.cluster_server
        namespace = "crossplane-system"
      }
      # Git is the source of truth: prune offboards a role when its YAML is removed
      # (gated by CODEOWNERS on gitops/roles). ServerSideApply so a future status
      # subresource is not stripped by selfHeal.
      syncPolicy = {
        automated = {
          selfHeal = true
          prune    = true
  ...
  depends_on = [kubernetes_manifest.roles_project]
}
