...
## Mutual authentication (SPIFFE workload identity)

Cryptographic workload identity ([ADR-057](../../../docs/adrs/057-service-identity-and-east-west-zero-trust.md) Phase 2). Set `mutual_auth_enabled = true` to run Cilium **mutual authentication** backed by the **embedded SPIRE** (`spire_install`), which issues SPIFFE SVIDs per workload. This authenticates _who_ is talking (the encryption of the wire is Phase 1's WireGuard). Enabling stands up the SPIRE server (StatefulSet, in `cilium-spire`) + agents but is **inert** until a `CiliumNetworkPolicy` opts in with `authentication.mode: required`. `spire_persistence` (default true) persists the SPIRE datastore to a PVC via `spire_storage_class` (null = the cluster default; must be an encrypted class where an SCP enforces EBS encryption). Like encryption, enabling needs a **rolling Cilium restart**; verify via `cilium-dbg bpf auth list` (`AUTH TYPE: spire`).
...
| <a name="input_mutual_auth_enabled"></a> [mutual\_auth\_enabled](#input\_mutual\_auth\_enabled) | Enable Cilium service-to-service mutual authentication (SPIFFE identity). Sets authentication.enabled = true (REQUIRED — else auth-required policies deny) + authentication.mutual.spire.enabled. Off by default; enforcement applies only to CNPs that opt in with authentication.mode: required. | `bool` | `false` | no |
...
| <a name="input_spire_install"></a> [spire\_install](#input\_spire\_install) | Install Cilium's embedded SPIRE server/agent (namespace cilium-spire) as the SPIFFE identity source when mutual\_auth\_enabled = true. Set false to point at an external/standalone SPIRE. | `bool` | `true` | no |
| <a name="input_spire_persistence"></a> [spire\_persistence](#input\_spire\_persistence) | Persist the embedded SPIRE server datastore to a PVC (recommended). Needs a StorageClass — where an SCP enforces EBS encryption, an ENCRYPTED class (spire\_storage\_class or the cluster default). Set false for an in-memory datastore (ephemeral registrations; fine for spikes). | `bool` | `true` | no |
| <a name="input_spire_storage_class"></a> [spire\_storage\_class](#input\_spire\_storage\_class) | StorageClass for the embedded SPIRE server's data PVC when spire\_persistence = true. null = the cluster's default StorageClass (encrypted gp3 here). Set explicitly to override. | `string` | `null` | no |
...
- **Overlay + IMDS:** tenant IMDS (`169.254.169.254`) protection is defense-in-depth — an explicit `deny-imds-egress` CiliumNetworkPolicy (`egressDeny`, rendered per environment namespace by the Composition, #160) that holds regardless of masquerade/SNAT, _plus_ the node's IMDSv2 hop-limit=1 backstop (for which link-local must stay masquerade-excluded). Environment infra (incl. IMDS isolation) is provisioned by the Crossplane Environment Composition (ADR-046/048/067).
