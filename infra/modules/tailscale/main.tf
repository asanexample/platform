...
# ---------------------------------------------------------------------------
# Tailscale Ingress — exposes services on the tailnet with automatic HTTPS
# ---------------------------------------------------------------------------

resource "kubernetes_manifest" "ts_ingress" {
  for_each = local.create ? var.ingresses : {}
...
resource "null_resource" "ts_ingress_finalizer_cleanup" {
  for_each = local.create ? var.ingresses : {}

  triggers = {
    ingress_name = each.key
    namespace    = each.value.namespace
  }

  provisioner "local-exec" {
    when    = destroy
    command = "kubectl delete ingress ${self.triggers.ingress_name} -n ${self.triggers.namespace} --ignore-not-found 2>/dev/null || true"
  }

  depends_on = [kubernetes_manifest.ts_ingress]
}
