...
- **Cross-cluster `spoke_ingest` (P10)** — authenticated remote-write **and OTLP** ingest from observability
  spoke clusters into the hub store: every spoke's edge routes both `/api/v1/push` (Prometheus remote-write —
  Beyla/kube-state) and `/otlp/v1/metrics` (Mimir's native OTLP ingest — a spoke's OTel collector forwarding
  tenant app-SDK metrics, ADR-100).
- **OTLP → Prometheus naming convergence (ADR-100)** — every tenant's `limits` sets
  `otel_metric_suffixes_enabled` + `otel_keep_identifying_resource_attributes` +
  `promote_otel_resource_attributes` (a **CSV string**, not a list — the wrong type crashloops every Mimir
  component; verify any change against the running binary's `/config`, never guess) so OTLP-ingested metrics
  carry the same `..._seconds` names + `k8s_namespace_name`/`k8s_pod_name` labels as remote-write metrics.
  Gotcha: OTLP series still break out per-service by `job` (`<team>-<product>/<svc>`), not `service_name` —
  Mimir doesn't surface the latter as a label even when promoted.
...
| <a name="input_admin_all_datasource_tenants"></a> [admin\_all\_datasource\_tenants](#input\_admin\_all\_datasource\_tenants) | Tenants the admin all-tenants datasource (enable\_admin\_all\_datasource) federates over, `|`-joined into its static X-Scope-OrgID. Set to the FULL tenant set including per-team tenants (e.g. ["alpha","bravo","platform","preprod"]) — the cluster-based all\_tenants (default + extras) omits team tenants. Empty falls back to all\_tenants. | `list(string)` | `[]` | no |
| <a name="input_app_slos"></a> [app\_slos](#input\_app\_slos) | Per-app SLOs (ADR-056 / per-app SLOs → W11 error-budget freeze). Each entry renders Sloth-style multi-window<br/>burn-rate rules into an `app-slos` Mimir ruler namespace, loaded into the ruler tenant by the rules-sync. The<br/>`error_query`/`total_query` use a `{{window}}` placeholder (Beyla RED metrics filtered to the app's namespace).<br/>Produces `slo:current_burn_rate:ratio{sloth_id=...}` per app (the metric the freeze gate queries) + page/ticket<br/>burn alerts → hub Alertmanager. Usually registry-derived in the unit from every XEnvironment claim, any stage. | <pre>list(object({<br/>    id              = string # unique SLO id, e.g. "alpha-shop-prod-availability"<br/>    service         = string # service label, e.g. "app-alpha-shop"<br/>    slo_name        = string # e.g. "requests-availability"<br/>    objective       = number # e.g. 99.9<br/>    error_query     = string # PromQL numerator with {{window}}<br/>    total_query     = string # PromQL denominator with {{window}}<br/>    alert_name      = string # base alert name (CamelCase)<br/>    page_severity   = optional(string, "critical")<br/>    ticket_severity = optional(string, "warning")<br/>  }))</pre> | `[]` | no |
...
| <a name="input_enable_admin_all_datasource"></a> [enable\_admin\_all\_datasource](#input\_enable\_admin\_all\_datasource) | Break-glass admin datasource (#1269): when the read proxy is enforced (read\_proxy\_url set), also provision a `Mimir (admin — all tenants)` datasource that queries the gateway DIRECTLY with a static federated `X-Scope-OrgID`, bypassing the proxy. Needed because the proxy relies on Grafana forwarding the caller's OIDC token (X-Id-Token), a fragile path — when it breaks, admins lose all metrics. Deliberately un-proxied (re-widens the metrics bypass), so opt-in and only meaningful with enable\_federated\_datasource. Off by default. | `bool` | `false` | no |
