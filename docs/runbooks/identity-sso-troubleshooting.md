...
**Who's the IdP (as of 2026-06):** **Keycloak** (`https://keycloak.aws.refplat.org/realms/platform`). ArgoCD
talks to it directly; Backstage talks to it through **oauth2-proxy**. **Dex is legacy** — if a symptom points
at `sso.aws.refplat.org` (Dex), it's almost certainly a stale config, not the live path.
...
| Backstage: **logged out on every refresh** | oauth2-proxy not in front / cookie issue | [§5](#5-backstage-session-drops-on-refresh) |
...
- Backstage path: the **oauth2-proxy** client (`oauth2-proxy`) redirect URI is
  `https://backstage.aws.refplat.org/oauth2/callback`.
...
`oauth2-proxy` and `backstage` units add a **host-alias** mapping the issuer host to the Cilium gateway
ClusterIP. If that Service was recreated, the pinned IP is stale.

```bash
kubectl -n default get svc cilium-gateway-platform-gateway -o jsonpath='{.spec.clusterIP}'
# Compare to host_aliases[].ip in the oauth2-proxy / backstage unit; update + re-apply if changed.
...
This was the original problem oauth2-proxy fixes (#202). If it's back:

- Confirm the gateway routes Backstage **through** oauth2-proxy (`backstage` namespace; the route backend is
  the oauth2-proxy Service), and oauth2-proxy pods are healthy: `kubectl -n backstage get pods`.
- Confirm Backstage is using the **`oauth2Proxy`** sign-in provider (header-based), not a direct OIDC provider.
- oauth2-proxy session length is `cookie-expire`; `cookie-refresh` is intentionally `0` (no upstream refresh
  token). A session ending exactly at `cookie-expire` is expected, not a bug.
