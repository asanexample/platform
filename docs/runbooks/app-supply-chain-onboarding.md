...
> Why this matters: Kyverno runs `verify-images-product-<team>-<product>` and
> `verify-attestations-product-<team>-<product>` in **Enforce** mode in environment namespaces. A pod whose
> image isn't signed (and attested) for **your product** is **rejected at admission**. Background:
> [`../architecture/supply-chain-overview.md`](../architecture/supply-chain-overview.md).
...
- **`build-sign.yml`** — build from your Dockerfile → push to `team-<team>/<product>-<svc>` ECR →
...
`deploy` job that pins the digest. Replace the `service:` value and (deliberately) the pinned `trusted-ci` SHA.
...
The shared workflow signs under **its own** identity; your product is the `githubWorkflowRepository` cert extension.
