...
| 5c | **remove `tenant-envelope.yaml` + fix `tenant-control-plane.yaml` → XEnvironment + the `.kyverno-tests` harness** (a coupled set: the v2 envelope/control-plane kyverno tests use XTenant fixtures + an apiCall-mocked envelope test; needs new XEnvironment fixtures + a v3 environment-envelope kyverno test). **⚠️ found: tenant-control-plane still guards `v1alpha1/XTenant`, not XEnvironment** — low exposure (XRD is cluster-scoped → tenants lack create-RBAC) but defense-in-depth should match XEnvironment. tenant-envelope kept (inert) for now so the Kyverno test stays green | ⛔ TODO |
| 5d | dead policy MODULE vars (verify_subjects/tenant_registry_map/migrated_teams/tenant_hostname_patterns/attest_caller_repos/shared_signer_caller_repos/enable_httproute_guard) + their plumbing; the crossplane module `charts/teams` helm_release (inert, teams={}) — cosmetic, harmless empty values | ◻ cosmetic |
