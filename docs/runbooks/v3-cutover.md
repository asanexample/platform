...
| 5b | remove the rest of v2 — XTenant `xrd.yaml`, `composition-v2.yaml`(+wrapper), `tenant-envelope.yaml`, per-team `image-registries.yaml`/`httproute-hostnames.yaml` (Composition-owned in v3; inert now) + their module vars, per-team github-oidc roles, the v1alpha2 Team version block, dead policy-unit locals, **the `.tenant-api-tests` v2 render/schema cases** (interdependent — touches the crossplane chart + the render harness) | ⛔ TODO |
