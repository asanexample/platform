...
| 5e | remove `tenant-envelope.yaml` (v2 envelope) + add a v3 **environment-envelope kyverno behavioral test** (XEnvironment resources + Team/Product apiCall-mock values-files for the 8 #387 rules) → swap the `.kyverno-tests` envelope section. tenant-envelope kept inert until this lands so the Kyverno Policy Test stays green | ⛔ TODO |
