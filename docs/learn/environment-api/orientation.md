...
> **Who this is for:** platform engineers who want to understand, operate, or extend the machine that
> provisions environments. **Developers:** you don't need this — see [the 2-minute version](#developers-the-2-minute-version)
> at the bottom. **Before you start**, you should know roughly what a container is and that Kubernetes
> has "resources" (a Deployment, a Namespace) that you describe in YAML and it makes real. That's the
> whole floor. Everything else we build up here.
>
> **Already fluent in Crossplane?** Skip the journey — the [Reference](reference.md) is the terse lookup
> (schema, commands, gotchas) built for readers who already hold the model.
...
On this platform you write **one short YAML file**, open a pull request, and a few minutes after it
merges the entire footprint exists — and stays correct forever after. This doc is about the machine
that makes that true. We call it the **Environment API**.

## Why not just do it by hand?

Fair question — you could just give every team an account and let them make a namespace, an IAM role, and
a registry themselves. Plenty of places do. Do it by hand, though, and four things go wrong at once:
...
That's the case for building this at all: you don't scale a platform by adding people to a queue — you
scale it by encoding the golden path as an API, so the right way *is* the default way. The Environment
API is that idea made concrete. (It's one instance of a bigger thesis — why the *whole platform* exists —
which will get its own home in the portal.)
...
That's it. The "what you want" is a file called a **claim**. The machine is called a **Composition**.
And "keeping reality matching" — continuously, not once — is the part that makes this different from a
script.

The cleanest way to *feel* that last part is a **thermostat**. You never tell a thermostat "run the
heater for ten minutes" — you tell it the temperature you *want*, and it spends all day quietly closing
the gap: heat clicks on when the room drifts cold, off when it's warm. It never "finishes." Crossplane is
a thermostat pointed at infrastructure — you set the desired state, and it continuously drives reality
toward it. (Hold that thought; near the end you'll see that deleting a resource by hand just makes it
come back — the thermostat noticed the room went cold.)

Where the picture breaks — and it's worth being precise, because a metaphor you don't bound will mislead
you: a thermostat chases *one number* toward a *dial* you turn. A control plane reconciles *many*
resources toward a whole *declarative spec* — the "setting" is a file describing desired state, not a
temperature. What carries over is the *loop* (never finishes, always closing the gap); what doesn't is
"one dial, one number."
...
Hold onto that picture. Now let's watch a real one get built.

> **Quick check:** in one sentence, what's the difference between the *claim* and the *Composition*? (If
> it's not instant yet — the restaurant: your order ticket vs. the kitchen. We'll see both for real next.)

## Watch one get built: `alpha-shop-dev`

There is a real environment on the platform called `alpha-shop-dev`. That name isn't arbitrary — it's
three coordinates from the platform's ownership model: a **Team** (`alpha` — who owns it), a **Product**
(`shop` — the thing they're building), and a **Stage** (`dev` — where on the dev → prod ladder this copy
sits). An *environment* is exactly that: **one Product at one Stage**, and its name is always
`<team>-<product>-<stage>`. So `alpha-shop-dev` reads as "Team alpha's `shop` product, in dev."

That Team → Product → Stage model is a first-class concept in its own right — the
[domain model](../domain-model/orientation.md) is the module that teaches it; here we just need the
coordinates so the name makes sense. This environment has been running untouched for weeks; we'll follow
it end to end.
...
That's the **claim** — a statement of desired state. Note what's *not* in it: no account IDs, no
registry URLs, no IAM ARNs, no network CIDRs. The claim is the environment-facing *contract*; all the
platform-specific plumbing is filled in for you (we'll see where from). `XEnvironment` is the **kind**
of thing this is — think of it like a custom Kubernetes resource type the platform defines, the same
way Kubernetes defines `Pod` or `Namespace`. The definition of that type is called an **XRD** (a
Composite Resource Definition) — the schema that says an `XEnvironment` must have a `team`, a `product`,
a `stage`, and may have the rest.

Here's the whole cast in one picture — it's a **restaurant**. The **XRD** is the *menu*: what you're
allowed to order. Your **claim** (the file above) is the *order ticket* you hand in. And the thing that
reads your ticket and actually cooks the meal — coming up in step 3 — is the *kitchen*: the
**Composition**. You never tell the kitchen *how* to cook; you order, and a full plated meal comes out.
(The restaurant has one flaw as a picture: a real kitchen forgets your order the moment it's served —
this one never does. That's the thermostat half: it keeps the meal on your table, forever.)

Hold the two pictures together, with the right one on top: the **thermostat is the *idea*** — what the
system *does*, and the thing to remember if you remember nothing else. The **restaurant just names the
*players*** — who's involved. Verb first, cast second.

### 2. The claim reaches the cluster

Nobody runs `kubectl apply` by hand. The file is merged via a pull request, and
[**ArgoCD**](https://argo-cd.readthedocs.io/en/stable/) — the
platform's GitOps engine — notices the change and syncs it onto the cluster. (GitOps just means: git is
the source of truth, and a controller continuously makes the cluster match git.) After the sync, the
`XEnvironment` object exists on the **preprod** cluster:
...
everything it created hanging off it:
...
- an [**IAM Role**](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html)
  `Pod-alpha-shop-dev-web` and a
  [**PodIdentityAssociation**](https://docs.aws.amazon.com/eks/latest/userguide/pod-identities.html) —
  together, these are how the app's pod gets AWS permissions without any static keys,
- an [**ECR Repository**](https://docs.aws.amazon.com/AmazonECR/latest/userguide/what-is-ecr.html)
  `team-alpha/shop-web` (plus its lifecycle + cross-account pull policy) — the image registry, scoped to
...
```mermaid
flowchart TD
    A["Edit gitops/environments/alpha/shop/dev.yaml"] -->|"PR + merge"| B["git (source of truth)"]
    B -->|"ArgoCD syncs"| C["XEnvironment 'alpha-shop-dev'<br/>on the preprod cluster"]
...
We've been saying "the machine." The machine is **Crossplane**, and it's worth understanding at a high
level because it explains *why* the environment stays correct.
...
And the self-heal is the same mechanism from the other side — you can watch it. In a **throwaway
...
Close this doc and see if you can answer these. If you can, you've got it:

1. What's the difference between the **claim** and the **Composition**?
...
If any of those are fuzzy, the [reference doc](reference.md) has the full mechanism, a glossary, and the
gotchas. If they're solid, you can now explain how environments get built on this platform — which is
most of what anyone will ever ask you about it.

> 📸 **Screenshot:** Backstage's catalog view of `alpha-shop-dev` — the platform's *own* picture of the
> environment, listing the namespace, the `team-alpha/shop-web` registry, the `Pod-alpha-shop-dev-web`
