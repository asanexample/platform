...
> self-heal on a real cluster — needs a **learning sandbox** that's on the roadmap but **not built yet**.
> Until it lands, **this tutorial runs entirely offline** with `crossplane render`: no cluster, no
> Tailscale, no AWS, nothing you can break. That still gives you the *real* footprint and the *real*
> edit → render → diff loop. The steps marked **🔒 (sandbox)** are what light up once the sandbox ships.

**What you'll do:** render a real environment on your laptop, read its footprint, then change it twice and
watch the output move. ~15 minutes, entirely local. Assumes you've read [the orientation](orientation.md).

## Before you start

- The **`crossplane` CLI** — check with `crossplane version` (client + server lines).
- **Docker running** — `crossplane render` runs the pipeline functions in containers.
- The platform repo checked out. All commands below run from the Environment Composition's test harness:
...
You'll see a stream of YAML documents — **the whole footprint that one claim produces**. That's the same
thing the platform would create on a cluster; you're just seeing it rendered offline.
...
Open `/tmp/my-env.yaml` and change `stage: dev` to `stage: test`. **Predict first:** which names change?
Then re-render the copy:
...
**stays `team-alpha/demo-web`**. If you predicted that, you've got it: names are product + service
...
model's coordinates, and grew the footprint by adding a service. No cluster, nothing to break — but the
output is exactly what the platform would build.

## Next — 🔒 when the learning sandbox is live

These are the steps that need the sandbox (not available yet):

- **Apply** your claim and watch **ArgoCD** sync it into a real namespace.
- **`crossplane resource trace`** it and watch the tree go `Ready`.
- **Delete** the ECR repo by hand and watch Crossplane **recreate it** — the self-heal you can only read
  about today.
