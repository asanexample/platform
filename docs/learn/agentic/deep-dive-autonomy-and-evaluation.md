...
> Assumes the [agentic orientation](orientation.md) — you know that an agent here is an `XAgent` claim, that
> it's boxed in on four axes, and that today it is strictly **propose-only**. This dive teaches the *one*
> hard idea the orientation flagged and deliberately abstracted: **how an agent could earn the right to
> act — and why almost none of that machinery exists yet.** If you only remember one sentence: *the ladder
> is a drawing; the only rung that's been built is a tape recorder.*

## The honest headline, first

Everything below describes a design. [ADR-086](../../adrs/086-autonomous-agent-access.md) — the document
that specifies how autonomy would work — carries the status header **"Proposed (draft / sketch —
2026-06-27)"**, and it means it. The grader doesn't exist. The reversibility registry doesn't exist. The
action-time policy engine doesn't exist. The `autonomy.mode` field in the live API is **schema-locked so it
*cannot express* anything but propose-only**. There is exactly **one** shipped artifact in this entire
topic, and it doesn't govern anything: a hardened S3 bucket that records incidents so that *someday*, if the
rest gets built, there's a corpus to grade against.

So this dive has two halves that must not be confused: **the design** (what earning autonomy would look
like) and **the one built piece** (the capture substrate). We'll keep the line between them loud, because
this is the single easiest area in the whole platform to oversell.
...
message. But a suggestion-only agent can't *auto-remediate* — restart a wedged pod at 3 a.m., scale to
absorb load, roll back a bad deploy — where waiting for a human to click approve defeats the entire point of
...
The load-bearing reframe — memorize this one, it's the whole philosophy:
...
reversible, proven bounds is another. The design swaps the first for the second on carefully chosen
actions — it never removes the rail.

## The design: autonomy is graduated, per-class, earned, and machine-bounded

The core decision (ADR-086) is a single sentence with four adjectives that each do work:

> Autonomy is **graduated, per-action-class, earned, and machine-bounded — never a global property of an
> agent.**

Read it slowly. There is no "autonomous agent." There is *this agent, autonomous on this one action-class,
because it earned it, within these machine bounds.* An agent can be autonomous on "restart a pod" and still
strictly propose-only on everything else, simultaneously.

*Callback to the orientation's metaphor: it's a **graduated driver's license.*** A new driver earns
privileges one skill at a time — learner's permit (supervised) → provisional (measured) → full — and a
violation **demotes** them. **Where it breaks:** a driver's license is a property of the *person*; here the
...
Two of these deserve unpacking, because they're the parts most likely to be built wrong.
...
this in a **platform-owned classification registry** — **never author-set, never agent-set** — that
classifies each class by reversibility × blast radius:
...
by a **policy decision at the action boundary** — the trusted code that actually holds the capability —
**not a central policy engine the agent calls out to**. Why does that matter? A central decision point the
agent queries is one the agent (or an injection riding it) can try to *talk past*; keeping the check at the
boundary that owns the capability means the code that would do the thing is the code that refuses — no "may
I?" round-trip to spoof. A deny doesn't error — it **degrades to propose + human gate**, the behavior you
have today.
...
## The status, verified against the code — this is the ruthless part
...
itself refuses the word. The orientation's "you might think you can set `autonomy.mode: autonomous`" is
answered by the OpenAPI validator, not by a human reviewer.
...
signal made visible — and **nothing emits that metric.** It's a dial wired to a sensor that hasn't been
installed. (Note even the *example* class in the title is `file-ticket`, a low-stakes reversible action — not
"remediate prod." The design's conservatism shows even in the mock.)

## The one thing that's real: the forward-capture substrate

Here is the built piece, and it's a good piece of engineering precisely *because* it does so little. Merged
as **#1074** (2026-07-01), it is the module
[`agent-eval-store`](https://github.com/asanexample/platform/blob/main/infra/modules/aws/agent-eval-store/main.tf)
plus its live unit, which stands up a single S3 bucket: **`platform-agent-eval-corpus`**. It captures real
triage episodes as fixtures — `{alert-group, telemetry snapshot, structured label, rubric}` — and does
**nothing else**. It **captures; it does not grade.**

### Why build the recorder before the examiner exists

*Callback to the orientation's metaphor: it's a **flight-data recorder installed before the plane is
certified for autopilot.*** You run the black box on every human-flown flight *now* — not because autopilot
is coming next week, but because the moment you *do* want to certify it, you'll need thousands of real flights
on tape, and you cannot record a flight that already landed.
...
forward, always-on, at incident time. The memory line for this is *"build the pipeline, not a corpus"* — the
bucket is the front of a pipeline that fills itself.
...
### The elegant bit: the write grant lives in the *agent's* claim, not the bucket

This is the detail worth stealing. The agent's permission to *write* to the corpus is **identity-based, in
its own `XAgent` claim** —
...
`UseEvalCorpusKey` — **not** in the bucket's resource policy. Why? A chicken-and-egg: the Composition *mints*
the agent's Pod-Identity role ARN, so it isn't known when you're writing the bucket's Terraform. Granting
from the identity side (the claim) instead of the resource side (the bucket) sidesteps that entirely — the
bucket name is deterministic, so the claim can name it, and the role grants itself access when the
...
the live triage agent, and the mechanism is beautifully cheap: the Slack triage card carries
**accept / correct / dismiss** buttons, and **the human's click *is* the eval label.** Each verdict
increments `triage_feedback_total{verdict, triage_disposition}`, and the "Accept-rate by disposition" panel
divides accepts by all verdicts *per disposition* — the **calibration** signal. (The dispositions are
`confident_lead` / `weak_leads` / `insufficient_evidence`; a low accept-rate on `confident_lead` means the
agent is over-confident.) We don't re-teach those metrics here — they belong to
...
not "promoted"). And calibration is scored, not just correctness: **a correct *abstention* is rewarded; a
confident-but-wrong root cause is scored *worse* than admitting uncertainty.** Feasibility was proven by a
spike on **2026-06-24/25** on a **12-fixture corpus** — result **GO** — which is exactly why the capture
substrate got built. But "GO on feasibility" is not "grader shipped"; the grader still lives in the backlog.
