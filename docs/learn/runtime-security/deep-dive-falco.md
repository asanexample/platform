...
Falco is the platform's runtime threat detector — the motion sensor inside the house. The door locks and
the ID check (admission + signed images) decide who gets *in*; Falco watches what happens once they're
inside, and it detects — it doesn't block. The [runtime-security orientation](orientation.md) sets up that
frame in full, at Stop 3, "watch behavior." This dive opens the sensor itself: how a syscall becomes an
alert, why it runs as a privileged per-node DaemonSet, what the live stream actually says right now, and
why the alarm is wired to no one on purpose.
...
One property shapes everything downstream: **Falco is detection, not prevention.** It observes a
...
automation) acting on that alert. That's why the whole rollout is sequenced the way it is.
...
This platform runs `modern_ebpf` — hard-coded in both live units and the module default. The reason is
...
every AMI upgrade. And it coexists with [Cilium](../foundations/deep-dive-the-cluster.md), which is
...
Verify it yourself: this platform runs the default upstream Falco ruleset, unchanged.
...
starting point, and it's the root of the tuning story below.
...
Read it top to bottom and the whole mechanism is visible: a `connect` syscall, from a process named
...
normal for *this* cluster. That's the headline lesson: an untuned ruleset is a firehose of
...
Falco detects. Who gets told? That's **falcosidekick**, the alert-routing fan-out layer. It is deployed —
...
Turning it on is a one-variable change — `falcosidekick_config = { sns = { topicarn = "…" } }`
...
The two threads tie together here. The default ruleset catches the genuinely bad — shell-in-a-container, a read of
