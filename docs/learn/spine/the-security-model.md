...
of what's missing. That tally moves as the posture matures. A security posture is a state, not a monument.
...
- **Account isolation + SCPs** *(built)* — preprod, prod, platform, and mgmt are separate AWS accounts. An
  ...
  make whole categories of action impossible org-wide
...
- **Audit** *(partial)* — CloudTrail records API activity on
...
- **Private API** *(built)* — the cluster API is private-only,
...
- **RBAC** *(built)* — least-privilege, per-team; the
  platform-engineer access model itself is read-and-operate, not author
  — even operators don't hold standing
...
- **Network policy — Cilium** *(built)* — pod-to-pod traffic is default-restricted, so a compromised pod
  can't freely roam the cluster. And pods are explicitly denied egress to the instance-metadata service
  (IMDS) *(built, live on preprod)* — a `deny-imds-egress` policy in every environment namespace closes a
  classic credential-theft / SSRF path, so IMDS protection doesn't rest solely on the node's IMDSv2
  hop-limit.
...
- **East-west encryption + mutual auth — [Cilium](https://cilium.io/) WireGuard + SPIFFE/SPIRE** *(built)* —
  the east-west (service-to-service) half of zero trust.
...
- **Signed + attested images** *(built)* — every image is cosign-signed and carries SLSA provenance,
  and Kyverno re-verifies at admission —
...
- **Secret scanning — gitleaks** *(built, live)* — a fail-closed CI job (plus a pre-commit hook) scans full
  ...
  So "someone pasted a key into a file" is caught by a scanner, not just by review.
...
- **Zero standing trust** — nothing is trusted by default or permanently: no long-lived keys (Pod Identity
  is short-lived), no standing admin (temporary-power expires), no unsigned image, no implicit network reach, no
...
| **Cross-cutting** | No **pentest / red-team** program; **compliance evidence** aspirational | Controls aren't adversarially validated or attested |
| **Cross-cutting** | **Backup / DR** partial — no tested restore/DR drills, no cross-region copy, RTO/RPO unvalidated | CNPG databases are backed up (WAL + PITR to S3), but recovery hasn't been rehearsed and non-CNPG data isn't covered |

None of these are secrets — they're tracked and prioritized, and naming them *is* the maturity signal.
