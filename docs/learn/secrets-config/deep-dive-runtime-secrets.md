...
> Assumes the [Secrets & Config orientation](orientation.md) — the "best secret is one that doesn't
> exist" posture and the **two planes** (config sealed in git; runtime secrets vaulted-and-synced).
> This deep dive lives entirely on the second plane and teaches **one** mechanism properly: how a
> running workload gets a credential without ever holding a long-lived key, using **AWS Secrets
> Manager** and the **[External Secrets Operator](https://external-secrets.io/latest/)** (ESO). If
> you're already fluent, the terse version is in the [Reference](reference.md).
...
it's base64-encoded plaintext sitting in etcd. So the object itself buys you almost nothing; the entire
security question is **where the value came from** and **who could see it on the way in**. That single
question is what [ADR-019](../../adrs/019-external-secrets-operator.md) exists to answer, and it's worth
seeing the homes it *rejected*, because each one fails in an instructive way:

- **Secrets in Terraform state** (a `kubernetes_secret` resource). Works, but it couples secret
  *rotation* to an infra *apply* — to change a password you run a plan — and every value lands in state,
  visible to anyone with state-read access whether or not they need that secret. Wrong blast radius.
- **Sealed Secrets** (encrypt client-side, commit the ciphertext, decrypt in-cluster). GitOps-friendly,
  but the encryption key is **per-cluster**: recreate the cluster and every secret must be re-encrypted,
  and there's no bridge to a cloud store, so you maintain values in *two* places.
...
**The decision:** AWS **[Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/intro.html)**
is the source of truth; **ESO** syncs a *named* secret out of it into an ordinary Kubernetes Secret that
any pod mounts unmodified. This is the deliberate **opposite plane** from
[ADR-066](../../adrs/066-sops-encrypted-config-secrets.md)'s SOPS-in-git: that plane holds *static
bootstrap identifiers the IaC reads at plan-time*; this one holds *rotating runtime credentials a live
pod consumes*. Never cross them.

> **Back to the orientation's metaphor** — the *bank safe-deposit box and a courier*. The credential
> lives once in the access-controlled, audited box (Secrets Manager). ESO is the courier who fetches a
> **copy** on a schedule and leaves it exactly where the pod expects it, so the pod never needs the
> vault key. **Where it breaks:** a real courier carries a key to the box on their belt — the whole
> point below is that *this* courier carries **no** key of its own.
...
(cheaper, for lower-sensitivity values). The **critical** detail is what the manifest *doesn't* contain:
there is **no `auth` block**. In the classic ESO tutorial the store carries an
`auth.jwt.serviceAccountRef` pointing at credentials. Here that stanza is gone — the store authenticates
as **ESO's own controller pod identity** (the `external-secrets` ServiceAccount bound to an IAM role via
...
It's a `ClusterSecretStore` (cluster-scoped — it can write a Secret into *any* namespace), not a
namespaced `SecretStore`. That's what lets a single store serve Grafana, ArgoCD, Backstage, and the rest
from one place.
...
slip: *"sync this remote key into a k8s Secret in my namespace, refreshed hourly."* The fields that
matter:
...
ESO reads the remote **as its Pod-Identity role**, materializes the k8s Secret, and the pod consumes it
via `envFrom` or a volume mount like any other Secret.
...
- A role **trusted by `pods.eks.amazonaws.com`** for `sts:AssumeRole` + `sts:TagSession` — the Pod
  Identity trust shape, no OIDC provider, no IRSA annotation on the ServiceAccount.
- A **read-only** policy: `secretsmanager:GetSecretValue`, `DescribeSecret`, `ListSecretVersionIds`
  scoped to `arn:aws:secretsmanager:*:<platform-acct>:secret:<prefix>/*`, plus the SSM
  `GetParameter`/`GetParameters`/`GetParametersByPath` equivalents. **No write, no delete** — a
  compromised ESO can *read* within its scope, never mutate the store.
- An `aws_eks_pod_identity_association` binding that role to the `external-secrets` ServiceAccount.

The Helm chart is pinned to **0.14.3**. Put together: the only thing standing between the cluster and
Secrets Manager is a short-lived, per-pod STS credential with read-only reach into one path prefix.

**Quick check:** where does ESO's AWS credential come from, and why is that better than an access key in
a `Secret`? *(Answer: EKS Pod Identity mints it per-pod from the ServiceAccount, short-lived and
auto-rotated — so there's no static key to leak, and the secrets bridge itself needs no secret.)*

## Worked example, traced end to end — Grafana's OIDC client secret

Follow **one real credential** across three systems. Grafana logs users in through Keycloak, so it needs
its OIDC **client secret** at runtime.

**1 — the writer.** The `keycloak-config` unit creates the Grafana OIDC client in Keycloak *and* writes
its client secret to Secrets Manager at **`platform/keycloak/grafana-oidc`**. Terraform owns both sides
of this value — the archetype of a cleanly-rotatable secret.

**2 — the delivery slip.** The
...
**3 — the running truth.** It's synced right now:
...
[`keycloak`](https://github.com/asanexample/platform/blob/main/infra/modules/keycloak/main.tf) module:
it generates a `random_password` (32 chars, alphanumeric), stores it as the source of truth at
**`platform/keycloak/admin`** as a `{username, password}` JSON blob, and its `keycloak-admin`
`ExternalSecret` maps both keys out — a clean two-key delivery slip. Same courier, but here Terraform
*mints* the secret rather than a third party.

## Naming and scoping — two layers of blast-radius control

**Naming** is `<domain-prefix>/<subsystem>/<name>`; every platform secret sits under the `platform/`
prefix (`platform/keycloak/grafana-oidc`, `platform/keycloak/admin`). Live, **16 ExternalSecrets** sync
across **8 namespaces**, all `SecretSynced`:
...
1. **IAM prefix scope.** The module's `secret_path_prefix` variable *defaults to `*`* — wide open — and
   the [live platform unit](https://github.com/asanexample/platform/blob/main/infra/live/aws/platform/us-east-1/platform/external-secrets/terragrunt.hcl)
   **narrows it** to `platform` (and `/platform` for SSM). So ESO on the hub can read **only**
   `platform/*`. The teaching point: the module ships permissive and the *unit* tightens it — always read
   the unit, not just the module default.
2. **Per-tenant path + a Kyverno backstop.** The *designed* tenant layout paths secrets at
   ...
   is **designed, not built** — see the honest caveat below.

## Keyless-first — ESO is the fallback, not the default

The point of the orientation's *eliminate-it* move is that most credentials **don't exist**. ESO is only
for the irreducible remainder — a third-party token you didn't mint. The hierarchy:
...
ESO itself is the proof of the principle: it reads Secrets Manager via **Pod Identity**, so *the secrets
bridge has no secret*. Every row that reaches "No" is a credential you never store, leak, or rotate.
...
- **ESO is down.** Secrets already materialized keep working — they're plain Kubernetes Secrets, entirely
  independent of the controller once written. Only **new syncs and rotations pause**. The failure is
  *soft*: a Grafana pod already holding `grafana-oidc` keeps authenticating; you just won't pick up a
  rotated value until ESO recovers.
- **The webhook is unreachable.** ESO's validating webhook runs on `hostNetwork` (the EKS control plane
  can't route Cilium overlay pod IPs) and is set to **`failurePolicy: Ignore`** on purpose. A default
  `Fail` webhook would block *deletion* of every `ExternalSecret` when its backend is unavailable —
  stranding units on teardown with "no endpoints available." Fail-open here is a deliberate
  lifecycle-safety choice, not laziness.

## The tenant paved road — designed, not built (be honest)

Here's the caveat the orientation flagged, in full. **Today a tenant app that needs a `DATABASE_URL` or
an API key has no platform-provided way to get one.** Grep proves it: `ExternalSecret` appears **zero**
times under `gitops/`, and the Crossplane Environment Composition mints **none**. Tenants get *nothing*
runtime-secrets today; the platform's *own* services use ESO heavily.

The **designed** model ([ADR-070](../../adrs/070-tenant-app-config-and-secrets.md), Status: **Proposed**)
is the Vercel/Heroku shape — push code, set env vars, it runs:
...
- **One writer.** Backstage is the **sole broker** (its own Pod-Identity role scoped to tenant paths);
  ...
  `(team, product, stage)` as a JSON blob (avoids per-key cost cardinality).
- **Prod is gated.** Prod writes and reveals need `team-admin`/`release-approver`; reveal is per-key and
  audited, with **no bulk export**.

**What actually landed:** only the **XRD schema reservation**. The
...
- **ADR-019 says IRSA; the code says Pod Identity.** [ADR-019](../../adrs/019-external-secrets-operator.md)
  ...
  correct. A live reminder that ADRs are *leads, not proof* — verify against the code.
