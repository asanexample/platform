...
The app scaffold it renders into `./app` (step 2) is more than source + CI: its `k8s/base/` ships a default HPA (`k8s/base/hpa.yaml`, `autoscaling/v2`, targeting the Argo **Rollout**, CPU 70% utilization, `minReplicas: 1`/`maxReplicas: 10` — the prod overlay patches it to **min 2 / max 20** to satisfy the ADR-085 prod replica-floor policy; metrics-server, an EKS add-on, is the CPU source). Every new Service is elastic by construction — you opt *out* (delete the file), not in.

The PR lands two files from [`new-product/platform-skeleton/`](https://github.com/asanexample/platform/blob/main/scaffolder/templates/new-product/platform-skeleton): a `kind: Product` (`spec.repo: asanexample/<team>-<product>`, `tenancy: pooled`, `domains: []`) and a `kind: XEnvironment` for `<team>-<product>-dev` with one service (`serviceAccount: app-<team>`) and the image omitted. That `Product` entry does a lot of work: under ADR-069 registries-as-single-source, `argocd-apps`, `policy`, and `github-oidc` all derive the per-Product image-scope (`team-<team>/<product>-*`), delivery, and OIDC from it — one truth, no drift.
...
- **Live.** The BACK stack (ADR-046, Accepted — implemented P1–P3, #174) and ADR-062 self-service provisioning (Accepted — built + live, Phase 3, #278–285). `hello-world` (#279), `new-team` (#280), and `new-environment` (#281) landed first; `new-product`/`new-environment`/`new-resource` were proven end-to-end when `acme/shop` went the whole way (ADR-073, Accepted 2026-06-17 — all four engines S3/SQS/SNS/DynamoDB live). The promotion ladder (#377) is proven live (auto ≤ staging + gated prod). The `onboard`/`offboard-person` pair is the newest, leaning on the People Gate. The portal itself is verified running (`backstage-…` `1/1 Running`, serving `backstage.aws.refplat.org`).
...
- **Source of truth (ADRs + code):** [ADR-046](../../adrs/046-back-stack-for-developer-self-service.md) (BACK stack), [ADR-062](../../adrs/062-self-service-tenant-provisioning.md) (self-service provisioning), [ADR-069](../../adrs/069-delivery-source-of-truth-product-environment.md) (registries-as-single-source), [ADR-073](../../adrs/073-self-service-cloud-resources.md) (self-service resources), [ADR-032](../../adrs/032-pr-preview-environments.md) (preview domains). The templates themselves: [`scaffolder/templates/`](https://github.com/asanexample/platform/blob/main/scaffolder/templates) + [`location.yaml`](https://github.com/asanexample/platform/blob/main/scaffolder/templates/location.yaml). The hand-authored counterpart to every path here is the `environment-onboarding` house skill.
