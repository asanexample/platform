...
appear here (`.platctl.yaml` contains real ones — omitted; VPC CIDRs `10.100/10.101` are RFC-1918,
non-sensitive).
...
- **Verbs:** `bootstrap` (walk forward), `teardown` (reverse), `down`/`up` (park), `validate` (health),
  plus `kubeconfig`, `status`, `access` (workforce). All built + tested (unit tests per package).
...
- **The floor (below platctl):** `state-bootstrap` uses a *local* backend on first apply (can't
  store state in the bucket it's creating), makes the S3 bucket + DynamoDB lock table (its own state stays local).
...
- **Cluster spine:** `iam-roles`+`networking` → `eks` → `cilium` → `node-groups` →
...
- **Bootstrap-then-lockdown:** EKS API is private-only by default; bootstrap applies it with
...
- **Platform services:** `external-secrets`→`secret-stores` before ExternalSecret consumers; `keycloak`→
  `keycloak-config`→`argocd`/Grafana OIDC (OIDC direct to Keycloak); `policy` before
...
- **⚠️ Never apply/teardown from a git worktree:** a worktree's differing abs-path can flip a `null_resource`
  `when=destroy` provisioner that force-deletes **live** IAM/ECR. The path resolves via `git rev-parse` so
  it's guarded, but the rule stays — operate from the main checkout.
...
- **UP:** credential preflight (both profiles) → re-apply `node-groups` → start bastion → wait for cluster
  API before Karpenter (early apply orphans its Helm releases) → recreate NodePool+EC2NodeClass
...
- **Thesis:** git IS the platform (IaC + GitOps + committed SOPS) → destroy + rebuild from zero. Runbook: *"data
  ...
  teardown cycles. Payoffs: DR, drift-elimination, no-snowflake-proof, confidence.
...
- **Node-imbalance meltdown → descheduler:** post-
  ...
  (platform `*/15` 40/70; preprod `*/10` 50%).
- **DB-client startup-ordering → `platctl up` DB-recovery:** a client up before its
  CNPG DB never retries → restart exactly those (bounded ~12 min; the sweep hardened against a silent no-op on cold unpark).
- **Stale cross-VPC DNS → `platctl up` reconnect:** preprod ENIs rotate → ArgoCD dials dead IPs →
...
- **Discipline:** on any failure, fix the *source* + commit + PR (manual
  unblock OK, but land the durable fix). The tell it's real: a durable fix later fixed a bug *in an earlier durable fix*.
...
- **Maturing / rebuild-gated:** park/unpark "not yet stable" (living doc); Keycloak + v3 cutovers verified only
  offline (next rebuild = first live apply); bootstrap's own state stays local.
...
`docs/runbooks/cluster-scale-down-up.md`. Related: [Foundations](../foundations/README.md),
