...
> Assumes the [Foundations orientation](orientation.md), Stops 5 and 6 — you already know the
> shape: a fixed **system** node group as the floor, elastic **Karpenter** on top, a **descheduler**
> that rebalances, and **Tailscale** as the staff door into a private-only cluster. This dive opens
> each of those four boxes and shows the machinery — the *why-it's-shaped-this-way* and the exact
> places it bites. It's for a platform engineer who now wants to operate this layer, not just picture it.

The orientation gave you two metaphors for the compute split that we'll lean on throughout: the
**always-on generator** (the system group — the lights that can never go out) versus **renting
generators by the hour** (Karpenter — right-sized capacity, just-in-time, returned when quiet). Hold
both; almost every design choice below is one of them defending its job.
...
Here's the load-bearing constraint, and it's almost tautological once you see it: **Karpenter can't
provision the node it runs on.** Karpenter is a controller — a pod — and a pod needs a node. If the
*only* nodes were Karpenter-provisioned, then a cold cluster (or one where every node just got
consolidated away) would have no Karpenter running, and therefore nothing to bring a node back. It's the
generator that starts the other generators: it can't be one of the ones it starts.

So a small fixed group stays, and it's deliberately the home for everything that is *cluster-critical and
un-provisionable-by-Karpenter*:
...
- **Node-pinned DaemonSet pods** that simply *cannot* be served by an elastic node (a DaemonSet runs one
  pod per node; it has nowhere to "scale up" *to*).
...
That `t4g.large → t4g.xlarge` change hides the nastiest gotcha in this whole layer, and
...
Why does that matter more here than on a normal cluster? Because **the in-cluster Tailscale subnet router
gets evicted during that window** — and the subnet router *is* how you reach the private EKS API (Part D).
So a one-line instance-type edit, which reads like a trivial resize, briefly makes the private API server
unreachable from your laptop. The lesson the ADR draws: an `instance_types` change is a *deliberate,
monitored* step, planned like a small maintenance, not slipped into a routine apply.
...
whose whole purpose is hardening. Three details worth knowing:
...
2. **Encrypted root EBS — or the launch is *refused*.** The template sets `encrypted = true` on a 20 GiB
   ...
   (`DenyUnencryptedEbsOnLaunch`) will **block the instance launch entirely** if the volume is
   unencrypted — you'd see `not authorized to launch instances with this launch template`. The
   encryption isn't a hardening nicety; it's the price of admission enforced from *above* the account.
...
> **Quick check:** why is the system node group `desired = 2` on platform but `desired = 1` on preprod —
> and why can neither of them be zero while the cluster is *up*? (Answer: platform carries the heavy
> standing stack + observability backends and needs the RAM/CPU across two nodes; preprod carries none of
> that. Neither can be zero because Karpenter, CoreDNS, and the Cilium operator live here — zero system
> nodes means no controller to bring elastic nodes back. Parking scales it to zero only as a *full*
> shutdown, cluster-parking skill.)

---

## Part B — Karpenter: renting the right generator, and returning it

Now the elastic layer. When pods go `Pending`, [Karpenter](https://karpenter.sh/) reads *exactly* what
they need — CPU, memory, architecture, zone — and launches the cheapest EC2 instance that fits, **bin-
packing** multiple pending pods onto one node where it can. When a node goes empty, it **consolidates** —
terminates it. That live `r6g.medium` from Part A is one such node; its `NodeClaim` is Karpenter's record
of the rental:
...
Notice Karpenter picked `r6g.medium` — not a type anyone hard-coded. That's the whole point of D1 in
...
**never bin-packs or consolidates** — it only adds and removes whole groups. Karpenter is
**instance-flexible**: its `NodePool` expresses *requirements* (arch, families, capacity type, a memory
floor), not a fixed shape, and it picks the cheapest sufficient instance per request in seconds. On a
cost-tight, park-when-idle reference cluster, **consolidation is the decisive feature** — it's the "return
the generator when it's quiet" half of the metaphor, and Cluster Autoscaler simply doesn't have it.
...
This is the orientation's "wet paint" sign, expressed for nodes that appear at 3 a.m. Because the cluster
...
smaller than 8 GiB (that live `r6g.medium` is exactly 8 GiB). Why? The per-node DaemonSet slab — Cilium,
Beyla, Alloy, node-exporter, otel — eats a **fixed ~3.2 GiB** on *every* node regardless of workload. On
a 4 GiB `t4g.medium` that slab exhausts memory, the kubelet flaps `NotReady`, and the node churns. The
...
exemption via a sloppy `*-karpenter-*`. And crucially, this SCP change must be applied **before** Karpenter
first provisions, or the cluster can't build — a real rebuild-ordering dependency, not an afterthought.

> **Quick check:** you unpark preprod, Karpenter is running, but no elastic nodes ever appear and its logs
> show `403 UnauthorizedOperation` on `RunInstances`. Which layer is wrong — the NodePool, the IAM role
> policy, or something above the account? (Above the account: an SCP is denying the tagged launch. No IAM
> grant *inside* preprod can fix it; the exemption is at the Organization level.)
...
> A note on status: [ADR-093](../../adrs/093-descheduler-node-rebalancing.md) is still marked
> **Proposed**, but the descheduler is **built and live on both clusters** (shipped in #1106) — the ADR
> header simply lags. Verify against the cluster, not the ADR status; that's exactly the trap the
> [verification gate](../_mold.md) warns about. Live proof:
...
*Why* does everything pile onto one node? Because after a staggered unpark, the "emptiest node" — the one
...
orientation's **maître d'** — walking a lopsided dining room and moving parties to emptier tables.

The thresholds are per-cluster, and the *why* is subtle:
...
But the maître d' analogy has a critical caveat the orientation flagged: **only to a seat that fits, and
never mid-meal.** The [descheduler
...
**topology-spread** spreads one workload's replicas, **Karpenter** adds/removes nodes, and the
...
Here's the single most load-bearing line in the Tailscale config, and it's the orientation's **two drivers
grabbing one steering wheel**. The subnet router runs in **userspace mode** — `TS_USERSPACE=true`, set via
the ProxyClass:
...
The orientation named this one: **"`kubectl` times out even though I'm on Tailscale."** Split DNS points
...
One security note that ties Parts A–C together. Whether you arrive via Tailscale or SSM, the role
...
- **The 8 GiB floor is a kubelet-stability floor, not a preference.** The ~3.2 GiB DaemonSet slab exhausts
  a 4 GiB node and flaps it `NotReady`. `min_instance_memory_mib = 6144` is load-bearing.
