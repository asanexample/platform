...
| 11 | Identity & secret management | ✅ | **Keycloak OIDC** as the app IdP of record (ArgoCD, Backstage, Grafana sign-in; pluggable-IdP seam, ADR-053/059), AWS SSO/Identity Center for console/CLI, per-team IAM, **EKS Pod Identity** (ADR-041/047) + IRSA, external-secrets → Secrets Manager, cert-manager. | — |
...
2. **Observability** (#7, [#102]) — now **live** end-to-end: LGTM+Profiles (metrics/logs/traces/profiles),
   ...
   **multi-cluster and federated** (preprod spoke), with Grafana Keycloak SSO. The tail is **AWS CUR
   true-spend** and **P13 per-team read isolation** ([#590]).
