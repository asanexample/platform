...
target tenant model (ADR-049/053) and surrounding ADRs (054–057) now set direction ahead of the planned
rebuild.
...
| 2 | Self-service APIs/CLIs | 🟡 | `platctl` (ops); tenants are a declarative Crossplane `Tenant` claim reconciled by a Composition (ADR-046/048). *But claims are still applied via platform-engineer PRs, not a developer-facing portal (Backstage, P5). Target model: ADR-049 separates ownership/isolation/placement (Team/Tenant/Zone/Customer), [v2 schema](tenant-api-v2.md).* | [#88](https://github.com/asanexample/platform/issues/88) |
...
(today `team == tenant`; the target separates ownership from isolation from placement). These are now
...
|------------|:------:|-------------------------------|-----|
| Multi-tenancy model (ownership · isolation · placement) | 📐 | Team/Tenant/Zone/Customer v2 schema with reserved enterprise dimensions (data, recovery, key custody, lifecycle); today `team == tenant` | [049](../adrs/049-tenant-model-team-tenant-zone.md) · [schema](tenant-api-v2.md) |
...
| Compliance assurance & evidence | 📐 | Control-catalog-as-code → SOC2/PCI/HIPAA, continuous scanning, retention-locked per-tenant evidence; controls exist, *assurance* doesn't | [055](../adrs/055-compliance-assurance-and-continuous-control-evidence.md) |
