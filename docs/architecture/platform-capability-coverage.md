...
| 2 | Self-service APIs/CLIs | 🟡 | `platctl` (ops); tenants are a declarative Crossplane `Tenant` claim reconciled by a Composition (ADR-046/048). *But claims are still applied via platform-engineer PRs, not a developer-facing portal (Backstage, P5)* | [#88](https://github.com/asanexample/platform/issues/88) |
