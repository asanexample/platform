...
- **Keyless signing mechanics** (OIDC → Fulcio → Rekor, per-product identity): [`cosign-image-signing.md`](cosign-image-signing.md)
...
```text
 ...
 git push main  ─────────▶ build-sign job  (uses asanexample/trusted-ci/...build-sign.yml@<pinned-sha>)
                             ...
                             2. docker build + push  →  ECR team-<team>/<product>-<svc>:<sha>
...
| **Kyverno** | Admission policy engine | Enforces all of the above **at deploy time**, per product | `policy` module, both clusters |
...
`verify-attestations-product-<team>-<product>` requires *exactly* those predicate types. A mismatch (e.g. cosign v3's new
...
**Beyond the Build track:** image **signatures** (integrity + per-product identity) and the **CycloneDX SBOM**
...
(per-product gating via the `githubWorkflowRepository` cert extension, ADR-050), so they inherit the same
