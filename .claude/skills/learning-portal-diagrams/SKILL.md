...
| **A number/glyph inside a chip is invisible in dark mode, or a chip lost its per-section colour (all chips look the same gold)** | Claude Design put an accent **text-colour** class (`f_blue`/`f_green`/`f_amber`/`f_red`) on the chip **shape**. The dark bake maps that class to one saturated accent that **beats the inline `fill`** (CSS class > presentation attr), so the pale chip flattens to that accent — and a same-accent glyph on top goes invisible (amber number on amber chip). Fix: **drop the accent class from the chip shape** (and from a same-accent glyph inside it), keeping the inline `fill` — the generic dark remap then darkens the pale tint and brightens the glyph, restoring contrast. The processor **lints** this (accent class on a *light-tint* shape); a solid accent *badge* (dark fill, contrasting white text) is fine and not flagged. **Contrast trap:** small coloured glyphs on coloured chips are exactly what a normal-zoom dark-mode screenshot hides — check chip/number contrast explicitly. |
