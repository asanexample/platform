...
3. **Open a PR.** There is **no `CODEOWNERS` entry for `gitops/environments/`** — envelope-bounded claims are deliberately self-service. The gate is the **gitops Gate** required status check (`validate-environments.sh`): shape/hygiene, Team-exists, envelope dry-run (`restrict-environment-envelope`), schema + composition-render, the IAM deny-set, and aggregate per-team quota — backed at runtime by Kyverno `restrict-environment-envelope` admission on the cluster. On a green Gate it may auto-merge.
